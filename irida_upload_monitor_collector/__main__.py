...
import irida_upload_monitor_collector.config
...
def main():
    ...
    while(True):
        try:
            if args.config:
                try:
                    config = irida_upload_monitor_collector.config.load_config(args.config)
            ...
            uploaded_sequencing_runs = core.find_all_uploaded_sequencing_runs(config)
            uploaded_sequencing_runs_output_file = os.path.join(config['output_dir'], 'uploaded_sequencing_runs.json')
            with open(uploaded_sequencing_runs_output_file, 'w') as f:
                json.dump(uploaded_sequencing_runs, f, indent=2)
                f.write('\n')
            logging.info(json.dumps({"event_type": "write_uploaded_sequencing_runs_file_complete", "uploaded_sequencing_runs_file": uploaded_sequencing_runs_output_file}))
